...
async function loadNews() {
  ...
  try {
    ...
    console.error(error);
    statusEl.textContent = "Unable to load news.";
    newsGrid.innerHTML =
      `<p class="empty">News could not be loaded.</p>`;
  }
}

function renderNews(news) {
  countEl.textContent = `${news.length} stories`;

  if (!news.length) {
    newsGrid.innerHTML =
      `<p class="empty">No news found.</p>`;
    return;
  }
  ...
  newsGrid.innerHTML = news.map(item => `
    <article class="card">
      ...
      <p>${escapeHtml(item.description || "Latest news update.")}</p>
...
chatForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    addMessage(
      "Sorry, AI Assistant is not responding. Please try again.",
      "bot"
    );
  }
});
